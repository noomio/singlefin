...
#include "test_data_call_module.h"
...
==============================================================================*/
void dam_obj_set_signal(void)
{
  LOG_INFO("DAM_APP:dam_obj_set_signal \n");

  tx_mutex_get(dam_obj_param.dam_obj_mutex, TX_WAIT_FOREVER);

  DAM_OBJ_TASK_SET_SIG(dam_obj_param.dam_obj_signal,DAM_OBJ_SIGNAL_MASK);
...
=============================================================================*/
    ...
    if (DAM_OBJ_TASK_IS_SIG_SET(received_sigs, DAM_OBJ_SIGNAL_MASK))
    {
      ...
      /* DAM_OBJ Signal is set Match to the signal handler */
      if(received_sigs & DAM_SIG_DATA_CALL_CONN) 
      {
        LOG_INFO("DAM_APP:DAM_SIG_DATA_CALL_CONN signal\n");
        dam_dss_net_is_conn(dam_dss_hndl);
      }
      else if(received_sigs & DAM_SIG_DATA_CALL_NO_NET) 
      {
        LOG_INFO("DAM_APP:DAM_SIG_DATA_CALL_NO_NET signal\n");
        dam_dss_net_no_net(dam_dss_hndl);
      }
...
/*=============================================================================
Start Normal Data Call
==============================================================================*/

  dam_dss_start_datacall();
