...
extern int ob_count;
extern boolean app_reg_status = false;
...
qapi_Net_LWM2M_Data_t* lwm2m_object_add
(
...
qapi_Net_LWM2M_Instance_Info_t* lwm2m_instance_add
(
	qapi_Net_LWM2M_Instance_Info_t* head,
...
qapi_Net_LWM2M_Data_t *lwm2m_object_find
(
	qapi_Net_LWM2M_Data_t* head,
    uint16_t id
)
...
qapi_Net_LWM2M_Instance_Info_t* lwm2m_instance_remove
(
	qapi_Net_LWM2M_Instance_Info_t   *head,
...
qapi_Net_LWM2M_Instance_Info_t* lwm2m_instance_find
(
	qapi_Net_LWM2M_Instance_Info_t* head,
    uint16_t id
)
  ...
  if((obj_info.obj_mask & QAPI_NET_LWM2M_OBJECT_ID_E ))
  {
	...
	if(object != NULL)
	{
	  if((obj_info.obj_mask & QAPI_NET_LWM2M_INSTANCE_ID_E))
	  {
		...
		if(instance != NULL)
		{
		  result = qapi_Net_LWM2M_Decode_App_Payload(&op_data.obj_info,
													 op_data.payload,
													 op_data.payload_len,
...
qapi_Status_t m2m_create_object(void)
{
	...
	if (object_generic == NULL)
	{
		tx_byte_allocate(byte_pool_m2m, (VOID **)&object, sizeof(qapi_Net_LWM2M_Data_t), TX_NO_WAIT);
		if (object == NULL)
		{
			IOT_DEBUG("Memory allocation failure. \n");
			return QAPI_ERR_NO_MEMORY;
...
qapi_Status_t m2m_create_object_instance_app(qapi_Net_LWM2M_Ext_t *obj_buffer)
{
	...
	{
		result = qapi_Net_LWM2M_Create_Object_Instance(ext_object_handler_app, &lwm2m_data);
	}
      ...
      else
      {
        ...
        if(object != NULL)
        {
          ...
          if(instance != NULL)
          {
            ...
            tx_byte_allocate(byte_pool_m2m, (VOID **)&instance_info.id_info, sizeof(qapi_Net_LWM2M_Id_Info_t), TX_NO_WAIT);
            ...
            result = qapi_Net_LWM2M_Delete_Object_Instance(ext_object_handler_app, &instance_info);
...
qapi_Net_LWM2M_Instance_Info_t *obj_create_generic_sensor(qapi_Net_LWM2M_Data_t *lwm2m_data)
{
	...
	tx_byte_allocate(byte_pool_m2m, (VOID **)&instance, sizeof(qapi_Net_LWM2M_Instance_Info_t), TX_NO_WAIT);
	if (instance == NULL)
	{
		IOT_DEBUG("Memory allocation failure.\n");
		return NULL;
...
qapi_Status_t lwm2m_ext_read(int obj_id, int inst_id, int res_id)
{
	qapi_Net_LWM2M_Data_t *object = NULL;
	qapi_Net_LWM2M_Instance_Info_t *instance = NULL;
	qapi_Net_LWM2M_Instance_Info_t *instance_start = NULL;
	qapi_Net_LWM2M_Resource_Info_t *resource = NULL;
	int i;
	uint16_t resList1[] = {
							RES_M_SENSOR_VALUE,
...
qapi_Status_t lwm2m_ext_write(int obj_id, int inst_id, int res_id, char *value)
{
	...
	switch(resource->type)
	{
		case QAPI_NET_LWM2M_TYPE_STRING_E:
		{
			memcpy(resource->value.asBuffer.buffer, value, sizeof(value));
			IOT_DEBUG("Resource %d : Value %s \n",resource->resource_ID, resource->value.asBuffer.buffer);
			IOT_DEBUG("Resource %d : Value %s \n",resource->resource_ID, resource->value.asBuffer.buffer);
			break;
		}
