 ...
 * ----------   ---   ---------------------------------------------------------
 * 2018-01-19   leo   (Tech Comm) Edited/added Doxygen comments and markup.
 * 2017-12-11   svl    updated Doxygen comments.
 ...
 *            the parameter "handle" must be a user space address, otherwise
 ...
 *                       status is QAPI_OK, handle will contain the
 ...
 * - QAPI_ERROR                  -- An internal failure occured.
 ...
 *
 * @note1hang If this API is called from user space with MMU enabled,
 *            the parameter "buffer" must be a user space address, otherwise
 *            an invalid parameter error will be returned.
 *
 * @param[in] handle  Handle returned from qapi_FTL_Open().
 ...
 *                       It should be an uncached, physically contiguous DMA-BLE.
 *
 * @return
