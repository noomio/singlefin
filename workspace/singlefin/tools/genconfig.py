...
platform_required_provides = [
    'FIN_USE_OS_STRING'  # must be #define'd
]
...
compiler_required_provides = [
    ...
    'FIN_USE_COMPILER_STRING',    # must be #define'd
    'FIN_USE_VARIADIC_MACROS',    # may be #undef'd, as long as provided
]
...
def generate_duk_config_header(opts, meta_dir):
    ...
    ret.line('/*')
    ret.line(' *  fin_config.h configuration header generated by genconfig.py.')
    ret.line(' *')
    ...
    ret.line(' */')
    ret.empty()
    ret.line('#if !defined(FIN_CONFIG_H_INCLUDED)')
    ret.line('#define FIN_CONFIG_H_INCLUDED')
    ret.empty()
...
def add_genconfig_optparse_options(parser, direct=False):
    ...
    parser.add_option('--platform', dest='platform', default='THREADX', help='platform (default is THREADX)')
