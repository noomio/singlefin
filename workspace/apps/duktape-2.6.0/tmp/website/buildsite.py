...
def processApiDoc(doc, testrefs, used_tags):
    ...
    if doc.has_key('properties'):
        p = splitNewlineNoLastEmpty(doc['properties'])
        res.append('<div class="api-part">')
        res.append('<h2 class="api-proto">Properties</h2>')
        res.append('<pre class="ecmascript-code">')
        for i in p:
            res.append(htmlEscape(i))
        res.append('</pre>')
    ...
    if doc.has_key('events'):
        p = splitNewlineNoLastEmpty(doc['events'])
        res.append('<div class="api-part">')
        res.append('<h2 class="api-proto">Events</h2>')
        res.append('<pre class="ecmascript-code">')
        for i in p:
            res.append(htmlEscape(i))
        res.append('</pre>')
...
def generateApiDoc(apidocdir, apitestdir):
    ...
    del title_elem['id']
    title_elem.string = 'NoomioJS API'
    ...
    res += processRawDoc('api/compile.html')
...
def main():
    ...
    soup = postProcess(soup, apiincdirs, autoAnchors=True, headingLinks=True, duktapeVersion=duk_verstr, noomiojsVersion=noomiojs_verstr )
    ...
    print 'Generating dfota.html'
    soup = generateDfotaPage()
    soup = postProcess(soup, dfotadir, autoAnchors=True, headingLinks=True, duktapeVersion=duk_verstr, noomiojsVersion=noomiojs_verstr)
    writeFile(os.path.join(outdir, 'dfota.html'), soup.encode(out_charset))
    ...
    for i in [ 'robots.txt',
               'media/layer.png',
               'media/whom.png',
               'media/qefs-explorer.jpg',
               'media/qefs-explorer-files.png',
               'media/LTE OPEN EVB KIT1.1.png',
               'media/example-digout.png',
               'media/example-print.png',
               'media/module-lpwa-quectel-bg96-te-a-with-gnss.png',
               'media/board-devkit.png',
               'media/bsdiff.png',
               'media/bspatch.png',
               'media/bspatch-mcu.png',
               '../LICENSE.txt'
                ]:
