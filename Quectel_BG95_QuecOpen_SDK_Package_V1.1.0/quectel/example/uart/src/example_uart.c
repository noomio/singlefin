...
int quectel_uart_task_entry(ULONG para)
{
	...
	ret = tx_byte_pool_create(byte_pool_uart, "uart task pool", free_memory_uart_task, UART_BYTE_POOL_SIZE);
  	if(ret != TX_SUCCESS)
  	{
...
int quectel_task_entry(void)
{
	...
	ret = tx_byte_pool_create(byte_pool_uart_task, "Sensor application pool", free_memory_uart, UART_BYTE_POOL_SIZE);
	if(ret != TX_SUCCESS)
	{
		IOT_DEBUG("tx_byte_pool_create [byte_pool_sensor] failed, %d", ret);
		return ret;
