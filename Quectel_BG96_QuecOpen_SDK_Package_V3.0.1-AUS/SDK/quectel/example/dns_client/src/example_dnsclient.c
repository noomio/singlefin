...
void dns_network_indication_cb(qapi_Device_Info_Hndl_t device_info_hndl, const qapi_Device_Info_t *dev_info)
{
	...
	{
		DNS_UART_DBG( "~ dns_network_indication_cb OK [%d]\n", dev_info->id);
...
void qt_dns_init_device_info(void)
{
	...
	if(status != QAPI_OK)
	{
		DNS_UART_DBG( "~ qapi_Device_Info_Init fail [%d]", status);
	}
	else
	{
		DNS_UART_DBG( "~ qapi_Device_Info_Init OK [%d]", status);
	}
}
