...
//#define HTTP_IPV6_ONLY //if not define HTTP_IPV6_ONLY ,use IPV4 type
/*===========================================================================
                               FUNCTION
===========================================================================*/
...
int http_set_dnsc_ipv6(void)
{
    ...
    HTTP_UART_DBG("<--- dns_client_session --->\n");
    ...
    for (i = 0; i < j; i++)
    {
        ...
        inet_ntop(AF_INET6, &info_ptr->dnsp_addr_s.addr.v6_addr64, first_dns,sizeof(first_dns));
        HTTP_UART_DBG("Primary DNS IP: %s\n", first_dns);

        memset(second_dns, 0, sizeof(second_dns));
        inet_ntop(AF_INET6, &info_ptr->dnss_addr_s.addr.v6_addr64, second_dns,sizeof(second_dns));
        HTTP_UART_DBG("Second DNS IP: %s\n", second_dns);
    }

    /* Start DNS service */
    HTTP_UART_DBG("qapi_Net_DNSc_Is_Started");
    e = qapi_Net_DNSc_Command(QAPI_NET_DNS_START_E);
    HTTP_UART_DBG("Start DNS service, ret: %d\n", e);

    /* Get current active iface */
    memset(iface, 0, sizeof(iface));
    qapi_DSS_Get_Device_Name(http_dss_handle, iface, 15);
    HTTP_UART_DBG("device_name: %s\n", iface);

    /* Add dns server into corresponding interface */
    qapi_Net_DNSc_Add_Server_on_iface(first_dns, QAPI_NET_DNS_V6_PRIMARY_SERVER_ID, iface);
    qapi_Net_DNSc_Add_Server_on_iface(second_dns, QAPI_NET_DNS_V6_SECONDARY_SERVER_ID, iface);
    return 0;
}
#ifdef QUECTEL_HTTPS_SUPPORT
...
int http_ssl_config(SSL_INST *ssl)
{
	...
	HTTP_UART_DBG("--->http ssl config: New SSL Object ssl->sslCtx - %lu", ssl->sslCtx);
...
static int http_download(char* host, uint16 port, uint8* file_name)
{
	...
	memset(&so_linger, 0x00, sizeof(so_linger)); 
	tx_byte_allocate(byte_pool_httpm, (void*)(&(httpc_cfg.sock_options)), sizeof(qapi_Net_HTTPc_Sock_Opts_t), TX_NO_WAIT);

	memset(httpc_cfg.sock_options, 0x00, sizeof(qapi_Net_HTTPc_Sock_Opts_t));
#ifndef HTTP_IPV6_ONLY
        httpc_cfg.addr_type = AF_INET;
#else
        httpc_cfg.addr_type = AF_INET6;
#endif

	so_linger.l_onoff = 1; 
	httpc_cfg.sock_options_cnt = 0;		//if we don't set this, following setting will not works
	httpc_cfg.sock_options->level = SOL_SOCKET; 
	httpc_cfg.sock_options->opt_name = SO_LINGER; 
	httpc_cfg.sock_options->opt_len = sizeof(so_linger); 
	httpc_cfg.sock_options->opt_value = &so_linger;
	httpc_cfg.max_send_chunk = 2000;
	httpc_cfg.max_send_chunk_delay_ms = 10;
	httpc_cfg.max_send_chunk_retries = 200;
	httpc_cfg.max_conn_poll_cnt = 15;
	httpc_cfg.max_conn_poll_interval_ms = 500;
    
	ret = qapi_Net_HTTPc_Configure(http_hadle, &httpc_cfg);
    if(ret != QAPI_OK)
    {
		HTTP_UART_DBG("HTTP config ret[%d]", ret);
		return -2;
    }

	ret = qapi_Net_HTTPc_Pass_Pool_Ptr(http_hadle, byte_pool_httpm);
    
    do
    {
        HTTP_UART_DBG("HTTP connecting \n");
...
void http_network_indication_cb(qapi_Device_Info_Hndl_t device_info_hndl, const qapi_Device_Info_t *dev_info)
{
	if(dev_info->id == QAPI_DEVICE_INFO_NETWORK_IND_E)
	{
		HTTP_UART_DBG( "~ http_network_indication_cb OK [%d]\n", dev_info->id);
		switch(dev_info->info_type)
		{
			case QAPI_DEVICE_INFO_TYPE_BOOLEAN_E:
				HTTP_UART_DBG( "~type[b] id[%d] status[%d]\n", dev_info->id, dev_info->u.valuebool);
				if(dev_info->u.valuebool == true)
				{
					tx_event_flags_set(http_nw_signal_handle, NW_SIG_EVT_ENABLE, TX_OR);
				}
				else
				{
					tx_event_flags_set(http_nw_signal_handle, NW_SIG_EVT_UNENABLE, TX_OR);
				}
			break;

			case QAPI_DEVICE_INFO_TYPE_INTEGER_E:
				HTTP_UART_DBG( "~type[i] id[%d] status[%d]\n", dev_info->id, dev_info->u.valueint);
			break;

			case QAPI_DEVICE_INFO_TYPE_BUFFER_E:
				HTTP_UART_DBG( "~type[s] id[%d] status[%s]\n", dev_info->id, dev_info->u.valuebuf.buf);
			break;

			default:
				HTTP_UART_DBG( "~[Invalid][%d]\n", dev_info->id);
			break;
		}
	}
}

void qt_http_init_device_info(void)
{
	...
	if(status != QAPI_OK)
	{
		HTTP_UART_DBG( "~ qapi_Device_Info_Init fail [%d]", status);
	}
	else
	{
		HTTP_UART_DBG( "~ qapi_Device_Info_Init OK [%d]", status);
	}
}
...
int quectel_task_entry(void)
{
...
#ifdef NW_SIGNAL_ENABLE
    txm_module_object_allocate(&http_nw_signal_handle, sizeof(TX_EVENT_FLAGS_GROUP));
    tx_event_flags_create(http_nw_signal_handle, "nw_signal_event");
    tx_event_flags_set(http_nw_signal_handle, 0x0, TX_AND);

    qt_http_init_device_info();
    status = qapi_Device_Info_Pass_Pool_Ptr(device_info_hndl, byte_pool_httpm);
    HTTP_UART_DBG("[dev_info] status, status %d\n", status);
    status = qapi_Device_Info_Set_Callback_v2(device_info_hndl, QAPI_DEVICE_INFO_NETWORK_IND_E, http_network_indication_cb);
    HTTP_UART_DBG("[dev_info] qapi_Device_Info_Set_Callback, status %d\n", status);

    nw_sig_mask = NW_SIG_EVT_UNENABLE | NW_SIG_EVT_ENABLE;
    while(1)
    {
	    tx_event_flags_get(http_nw_signal_handle, nw_sig_mask, TX_OR, &nw_event, TX_WAIT_FOREVER);
	    HTTP_UART_DBG("NW SIGNAL EVENT IS [%d]\n", nw_event);

	    if (nw_event & NW_SIG_EVT_UNENABLE)
	    {
		    HTTP_UART_DBG("waiting for network enabled...\n");
		    tx_event_flags_set(http_nw_signal_handle, ~NW_SIG_EVT_UNENABLE, TX_AND);
	    }
	    else if (nw_event & NW_SIG_EVT_ENABLE)
	    {
		    HTTP_UART_DBG("network enabled!\n");
		    tx_event_flags_set(http_nw_signal_handle, ~NW_SIG_EVT_ENABLE, TX_AND);
		    break;
	    }
    }
#endif /* NW_SIGNAL_ENABLE */
    ...
    if(dss_event&DSS_SIG_EVT_CONN_E)
    {
        ...
        {
            /*if customers do not know use which IPV4 DNS server, they can get IPV4 DNS server from network,
              please refer to the code of adding IPV6 DNS server*/
#ifndef HTTP_IPV6_ONLY
            qapi_Net_DNSc_Command(QAPI_NET_DNS_START_E);
            qapi_Net_DNSc_Add_Server("223.5.5.5", 0);
            qapi_Net_DNSc_Add_Server("114.114.114.114", 1);
#else
            ret = http_set_dnsc_ipv6();
            if(ret < 0)
            {
                 HTTP_UART_DBG("HTTP set_dnsc_ipv_6 fail");
            }
#endif
