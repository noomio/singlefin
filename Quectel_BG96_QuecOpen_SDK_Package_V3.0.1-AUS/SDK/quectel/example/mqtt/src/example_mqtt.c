...
void mqtt_network_indication_cb(qapi_Device_Info_Hndl_t device_info_hndl, const qapi_Device_Info_t *dev_info)
{
	...
	{
		MQTT_UART_DBG( "~ mqtt_network_indication_cb OK [%d]\n", dev_info->id);
...
void qt_tcp_init_device_info(void)
{
	...
	if(status != QAPI_OK)
	{
		MQTT_UART_DBG( "~ qapi_Device_Info_Init fail [%d]", status);
	}
	else
	{
		MQTT_UART_DBG( "~ qapi_Device_Info_Init OK [%d]", status);
	}
}
