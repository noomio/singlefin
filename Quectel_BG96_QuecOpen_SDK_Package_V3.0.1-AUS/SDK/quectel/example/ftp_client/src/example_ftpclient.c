...
void* ftp_nw_signal_handle = NULL;	/* Related to nework indication */
...
void ftp_network_indication_cb(qapi_Device_Info_Hndl_t device_info_hndl, const qapi_Device_Info_t *dev_info)
{
	if(dev_info->id == QAPI_DEVICE_INFO_NETWORK_IND_E)
	{
		FTP_UART_DBG( "~ ftp_network_indication_cb OK [%d]\n", dev_info->id);
		switch(dev_info->info_type)
		{
			case QAPI_DEVICE_INFO_TYPE_BOOLEAN_E:
				FTP_UART_DBG( "~type[b] id[%d] status[%d]\n", dev_info->id, dev_info->u.valuebool);
				if(dev_info->u.valuebool == true)
				{
					tx_event_flags_set(ftp_nw_signal_handle, NW_SIG_EVT_ENABLE, TX_OR);
				}
				else
				{
					tx_event_flags_set(ftp_nw_signal_handle, NW_SIG_EVT_UNENABLE, TX_OR);
				}
			break;

			case QAPI_DEVICE_INFO_TYPE_INTEGER_E:
				FTP_UART_DBG( "~type[i] id[%d] status[%d]\n", dev_info->id, dev_info->u.valueint);
			break;

			case QAPI_DEVICE_INFO_TYPE_BUFFER_E:
				FTP_UART_DBG( "~type[s] id[%d] status[%s]\n", dev_info->id, dev_info->u.valuebuf.buf);
			break;

			default:
				FTP_UART_DBG( "~[Invalid][%d]\n", dev_info->id);
			break;
		}
	}
}
...
void qt_ftp_init_device_info(void)
{
	...
	if(status != QAPI_OK)
	{
		FTP_UART_DBG( "~ qapi_Device_Info_Init fail [%d]", status);
	}
	else
	{
		FTP_UART_DBG( "~ qapi_Device_Info_Init OK [%d]", status);
	}
}
...
#ifdef NW_SIGNAL_ENABLE
	txm_module_object_allocate(&ftp_nw_signal_handle, sizeof(TX_EVENT_FLAGS_GROUP));
	tx_event_flags_create(ftp_nw_signal_handle, "nw_signal_event");
	tx_event_flags_set(ftp_nw_signal_handle, 0x0, TX_AND);

	qt_ftp_init_device_info();
	status = qapi_Device_Info_Pass_Pool_Ptr(device_info_hndl, byte_pool_ftp);
	FTP_UART_DBG("[dev_info] status, status %d\n", status);
	status = qapi_Device_Info_Set_Callback_v2(device_info_hndl, QAPI_DEVICE_INFO_NETWORK_IND_E, ftp_network_indication_cb);
	FTP_UART_DBG("[dev_info] qapi_Device_Info_Set_Callback, status %d\n", status);

	nw_sig_mask = NW_SIG_EVT_UNENABLE | NW_SIG_EVT_ENABLE;
	while(1)
	{
		tx_event_flags_get(ftp_nw_signal_handle, nw_sig_mask, TX_OR, &nw_event, TX_WAIT_FOREVER);
		FTP_UART_DBG("NW SIGNAL EVENT IS [%d]\n", nw_event);

		if (nw_event & NW_SIG_EVT_UNENABLE)
		{
			FTP_UART_DBG("waiting for network enabled...\n");
			tx_event_flags_set(ftp_nw_signal_handle, ~NW_SIG_EVT_UNENABLE, TX_AND);
		}
		else if (nw_event & NW_SIG_EVT_ENABLE)
		{
			FTP_UART_DBG("network enabled!\n");
			tx_event_flags_set(ftp_nw_signal_handle, ~NW_SIG_EVT_ENABLE, TX_AND);
			break;
		}
	}
#endif /* NW_SIGNAL_ENABLE */
